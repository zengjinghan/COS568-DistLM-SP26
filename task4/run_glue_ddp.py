...
def is_distributed(args):
    return args.local_rank != -1 and dist.is_available() and dist.is_initialized()
...
def sync_gradients_gather_scatter(args, model):
    ...
    for param in model.parameters():
        ...
        dist.scatter(synced_grad, scatter_list=scatter_list, src=0)
...
def build_profiler(args):
    ...
    trace_path = os.path.join(
        args.output_dir,
        "{}_rank{}.json".format(args.profile_name, args.local_rank if args.local_rank != -1 else 0),
    )
...
def train(args, train_dataset, model, tokenizer):
    ...
    train_sampler = DistributedSampler(train_dataset) if is_distributed(args) else RandomSampler(train_dataset)
    ...
    for epoch_idx in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            ##################################################
            # TODO(cos568): print the loss for the first 5 mini-batches.
            if step < 5:
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    if is_distributed(args) and (not evaluate) and args.local_rank not in [-1, 0]:
    ...
    if is_distributed(args) and (not evaluate) and args.local_rank == 0:
        dist.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
...
def main():
    ...
    parser.add_argument("--world_size", type=int, default=1,
                        help="Number of workers participating in distributed training.")
    ...
    parser.add_argument("--profile_name", type=str, default="ddp_trace",
                        help="Base filename for the exported chrome trace.")
    ...
    if is_distributed(args):
        if args.device.type == "cuda":
            model = DDP(model, device_ids=[args.device.index], output_device=args.device.index)
        else:
            model = DDP(model)
